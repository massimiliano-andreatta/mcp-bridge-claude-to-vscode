...
export class DiffViewProvider {
  ...
  async saveChanges(): Promise<{
    ...
    Since these are new problems resulting from MCP Bridge's edit, we know they're
    directly related to the work he's doing. This eliminates the risk of MCP Bridge
    going off-task or getting distracted by unrelated issues, which was a problem
    with the previous auto-debug approach. Some users' machines may be slow to
    update diagnostics, so this approach provides a good balance between automation
    and avoiding potential issues where MCP Bridge might get stuck in loops due to
    outdated problem information. If no new problems show up by the time the user
    accepts the changes, they can always debug later using the '@problems' mention.
    This way, MCP Bridge only becomes aware of new problems resulting from his edits
  ...
  private async closeAllDiffViews() {
    const tabs = vscode.window.tabGroups.all.flatMap((tg) => tg.tabs).filter((tab) => tab.input instanceof vscode.TabInputTextDiff && tab.input?.original?.scheme === DIFF_VIEW_URI_SCHEME);
    for (const tab of tabs) {
  ...
  private async openDiffEditor(): Promise<vscode.TextEditor> {
    ...
    const diffTab = vscode.window.tabGroups.all.flatMap((group) => group.tabs).find((tab) => tab.input instanceof vscode.TabInputTextDiff && tab.input?.original?.scheme === DIFF_VIEW_URI_SCHEME && arePathsEqual(tab.input.modified.fsPath, uri.fsPath));
  ...
  getChangedRanges(): vscode.Range[] {
    ...
    for (const part of diffs) {
      ...
      if (part.added || part.removed) {
        const range = new vscode.Range(new vscode.Position(lineNumber, 0), new vscode.Position(lineNumber + (part.count || 0), 0));
