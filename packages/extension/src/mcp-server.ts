...
export class ToolRegistry {
  ...
  toolWithRawInputSchema(name: string, description: string, inputSchema: Tool["inputSchema"], cb: (args: unknown, extra: RequestHandlerExtra<any, any>) => ReturnType<ToolCallback<any>>) {
  ...
  tool<Args extends ZodRawShape>(name: string, description: string, paramsSchema: Args, cb: ToolCallback<Args>) {
    ...
    this._registeredTools[name] = {
      description,
      inputZodSchema: paramsSchema === undefined ? undefined : z.object(paramsSchema),
      callback: cb,
    };
  ...
  #setToolRequestHandlers() {
      ...
      (): ListToolsResult => ({
        tools: Object.entries(this._registeredTools).map(
          ([name, tool]): Tool => ({
            ...
            inputSchema: tool.inputSchema ??
              (tool.inputZodSchema &&
                (zodToJsonSchema(tool.inputZodSchema, {
                  strictUnions: true,
                }) as Tool["inputSchema"])) ?? { type: "object" as const },
          })
        ),
      })
    );
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request, extra): Promise<CallToolResult> => {
      ...
      if (tool.inputSchema) {
        ...
        const cb = tool.callback as (args: unknown, extra: RequestHandlerExtra<any, any>) => ReturnType<ToolCallback<any>>;
        return await Promise.resolve(cb(args, extra));
      } else if (tool.inputZodSchema) {
        const parseResult = await tool.inputZodSchema.safeParseAsync(request.params.arguments);
...
function registerTools(mcpBridgeC2V: ToolRegistry) {
  ...
  mcpBridgeC2V.tool(
    ...
    `.trim(),
    {
      severityLevel: z.enum(["Error", "Warning", "Information", "Hint"]).default("Warning").describe("Minimum severity level for checking issues: 'Error', 'Warning', 'Information', or 'Hint'."),
    },
    async (params: { severityLevel?: "Error" | "Warning" | "Information" | "Hint" }) => {
      const severityLevel = params.severityLevel ? DiagnosticSeverity[params.severityLevel] : DiagnosticSeverity.Warning;
